    ...
    <ol class="breadcrumb text-nowrap">
        ...
        @else
            ...
            @foreach ($folders as $index => $folder)
                ...
                @else
                    <li class="breadcrumb-item">
                        <a draggable="false" role="button" class="text-body" wire:click="goTo({{ $index }})"
                            wire:offline.class="disabled" wire:offline.attr="disabled" wire:loading.class="disabled"
                            wire:loading.attr="disabled">
                            <span wire:loading.remove wire:target="goTo({{ $index }})">
                                <span class="fas fa-folder fa-fw"></span>
                                {{ $folder['name'] }}
                            </span>
                            <span wire:loading wire:target="goTo({{ $index }})" class="w-100">
                                <span class="spinner-border spinner-border-sm"></span>
                                {{ $folder['name'] }}
                            </span>
                        </a>
